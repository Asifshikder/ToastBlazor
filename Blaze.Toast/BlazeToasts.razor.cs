...
namespace Blaze.Toast
{
    ...
    public partial class BlazeToasts
    {
        ...
        private ToastSettings BuildToastSettings(ToastLevel level, RenderFragment message, string heading, Action? onclick)
        {
            switch (level)
            {
                case ToastLevel.Error:
                    return new ToastSettings(ToastLevel.Error, string.IsNullOrWhiteSpace(heading) ? "Error" : heading, message,
                        "blaze-toast-error", ErrorClass, ErrorIcon, ShowProgressBar, MaxToastCount, onclick);

                case ToastLevel.Info:
                    return new ToastSettings(ToastLevel.Info, string.IsNullOrWhiteSpace(heading) ? "Info" : heading, message,
                        "blaze-toast-info", InfoClass, InfoIcon, ShowProgressBar, MaxToastCount, onclick);

                case ToastLevel.Success:
                    return new ToastSettings(ToastLevel.Success, string.IsNullOrWhiteSpace(heading) ? "Success" : heading, message,
                        "blaze-toast-success", SuccessClass, SuccessIcon, ShowProgressBar, MaxToastCount, onclick);

                case ToastLevel.Warning:
                    return new ToastSettings(ToastLevel.Warning, string.IsNullOrWhiteSpace(heading) ? "Warning" : heading, message,
                        "blaze-toast-warning", WarningClass, WarningIcon, ShowProgressBar, MaxToastCount, onclick);
            }
        ...
        private void ClearToasts(ToastLevel toastLevel)
        {
            InvokeAsync(() =>
            {
                ToastList.RemoveAll(x => x.BlazeToast == null && x.ToastSettings.ToastLevel == toastLevel);
                StateHasChanged();
            });
        }
        ...
        private void ClearCustomToasts()
        {
            InvokeAsync(() =>
            {
                ToastList.RemoveAll(x => x.BlazeToast is object);
                StateHasChanged();
            });
        }
    }
